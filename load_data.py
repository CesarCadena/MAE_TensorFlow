...
def load_set_data(data_seq):
    ...
    for i in range(0,n_seq):
        ...
        for j in range(0,data_set[i]['xcr'].shape[1]):
            frame['xcr1'] = copy(data_set[i]['xcr'][:,j])
            frame['xcg1'] = copy(data_set[i]['xcg'][:,j])
            frame['xcb1'] = copy(data_set[i]['xcb'][:,j])

            frame['xcr2'] = copy(data_set[i+n_seq]['xcr'][:,j])
            frame['xcg2'] = copy(data_set[i+n_seq]['xcg'][:,j])
            frame['xcb2'] = copy(data_set[i+n_seq]['xcb'][:,j])

            frame['xidsparse1'] = copy(data_set[i+2*n_seq]['xidsparse'][:,j])
            frame['xid1'] = copy(data_set[i+2*n_seq]['xid'][:,j])
            frame['xmask1'] = copy(data_set[i+2*n_seq]['xmask'][:,j])

            frame['xidsparse2'] = copy(data_set[i+3*n_seq]['xidsparse'][:,j])
            frame['xid2'] = copy(data_set[i+3*n_seq]['xid'][:,j])
            frame['xmask2'] = copy(data_set[i+3*n_seq]['xmask'][:,j])

            frame['sem1'] = copy(data_set[i+4*n_seq]['xss'][:,j])
            frame['sem2'] = copy(data_set[i+5*n_seq]['xss'][:,j])
