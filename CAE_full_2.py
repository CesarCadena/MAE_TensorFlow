...
#now (batch,9,30,8)
    ...
    depth_upsample2 = tf.image.resize_images(depth_conv4, size=(18,60),
                                   method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
#now (batch,18,60,8)
    depth_out=tf.layers.conv2d(inputs=depth_upsample2,filters=5,kernel_size=(3,3),padding='same',
...
optimizer=tf.train.AdamOptimizer(learning_rate).minimize(loss,var_list=var_full)
...
with tf.Session(config=config) as sess:
    sess.run(init)
    saver_sem.restore(sess,'./CNN_models/sem_50/sem.ckpt')
    saver_depth.restore(sess,'./CNN_models/depth_50/depth.ckpt')
    saver_rgb.restore(sess,'./CNN_models/rgb_50/rgb.ckpt')


    for ipochs in range(100):
        perm_indices=np.random.permutation(train_indices)

        for step in range(int(train_size/batch_size)):
            offset=(step*batch_size)%(train_size-batch_size)
            batch_indices=perm_indices[offset:(offset+batch_size)]
            l,_=sess.run([loss,optimizer],feed_dict={sem_inputs:sem_data[batch_indices],
        ...
        saver_full.save(sess,'./CNN_models/full_100/full.ckpt')
